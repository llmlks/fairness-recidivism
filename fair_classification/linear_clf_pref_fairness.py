    ...
    def fit(self, X, y, x_sensitive, cons_params=None):
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                w[k] = cvx.Variable(X.shape[1]) # this is the weight vector
                w[k].value = np.random.rand(X.shape[1]) # initialize the value of w -- uniform distribution over [0,1]
        else:
            w = cvx.Variable(X.shape[1]) # this is the weight vector
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                ...
                if self.loss_function == "logreg":
                    obj += cvx.sum(  cvx.logistic( cvx.multiply(-y_k, X_k*w[k]) )  ) / num_all # notice that we are dividing by the length of the whole dataset, and not just of this sensitive group. this way, the group that has more people contributes more to the loss
                    
                elif self.loss_function == "svm_linear":
                    obj += cvx.sum ( max_elemwise (0, 1 - cvx.multiply ( y_k,  X_k*w[k])) ) / num_all
            ...
            if self.loss_function == "logreg":
                obj += cvx.sum(  cvx.logistic( cvx.multiply(-y, X*w) )  ) / num_all
            elif self.loss_function == "svm_linear":
                obj += cvx.sum ( max_elemwise (0, 1 - cvx.multiply ( y,  X*w)) ) / num_all
            else:
                raise Exception("Invalid loss function")
        ...
        if cons_type in [1,2,3]: # 1 - pref imp, 2 - EF, 3 - pref imp & EF
            ...
            for val in set(x_sensitive):
                ...
                for k in w.keys(): # get the distance with each group's w
                    prod_dict[val][k] = cvx.sum(  max_elemwise(0, X_g*w[k])   ) / num_g
